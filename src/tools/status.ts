...
function formatStatus(v: GitStatusValue): string {
  const lines = [
    `Branch: ${v.branch}${v.ahead || v.behind ? ` (ahead ${v.ahead}, behind ${v.behind})` : ""}`,
  ];
  if (v.staged.length)
    lines.push(`Staged (${v.staged.length}):`, ...v.staged.map((s) => `  ${s.status} ${s.path}`));
  if (v.unstaged.length)
    lines.push(
      `Unstaged (${v.unstaged.length}):`,
      ...v.unstaged.map((s) => `  ${s.status} ${s.path}`),
    );
  if (v.untracked.length)
    lines.push(`Untracked (${v.untracked.length}):`, ...v.untracked.map((p) => `  ?? ${p}`));
  return lines.join("\n");
}
...
export const gitStatusTool = defineTool({
  name: "git_status",
  description:
    "Show the working tree status: current branch, ahead/behind, and staged, unstaged, and untracked files.",
  parameters: {
    cwd: {
      type: "string",
