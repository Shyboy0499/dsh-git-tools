...
function formatDiff(v: GitDiffValue): string {
  ...
  lines.push(
    `${v.total.files} file(s) changed, ${v.total.insertions} insertion(s), ${v.total.deletions} deletion(s)`,
  );
...
export const gitDiffTool = defineTool({
  name: "git_diff",
  description:
    "Show changes in the working tree (unstaged) or in the index (staged). Returns a stat summary, or the full patch when statOnly is false.",
  parameters: {
    cwd: {
      type: "string",
